...
class AcceleratorDetector:
                ...
                if torch.cuda.is_available():
                    for i in range(torch.cuda.device_count()):
                        try:
                            ...
                            # Validate that GPU kernel compute actually works
                            # (e.g. PyTorch nightly with unstable ROCm may segfault on kernel ops)
                            if not self._validate_device_compute(i):
                                logger.warning(
                                    f"ROCm device {i} ({device_props.name}) failed kernel"
                                    " validation - excluding from available devices"
                                )
                                continue
    ...
    @staticmethod
    def _validate_device_compute(device_index: int) -> bool:
        """
        Validate that GPU kernel compute actually works for the given device.

        Spawns a subprocess to test a simple kernel operation, because a segfault
        in the ROCm runtime cannot be caught with try/except in Python.
            ...
            result = subprocess.run(
                [
                    ...
                    (
                        ...
                        f"{hsa_override}"
                        "import torch;"
                        f"torch.zeros(1, device='cuda:{device_index}');"
                        "print('OK')"
                    ),
                ],
                capture_output=True,
